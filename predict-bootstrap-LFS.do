...
replace occupation = 4   if (employment_type == 2 & p4_12>2) | (employment_type == 4)  // private employee including unpaid family workers
...
global char_cat_uno "sector occupation"
...
save `HBS2016'
			...
			keep if year==2016
			replace wgt=wgt/4 if year==2016|year==2015
			...
			replace occupation=4 if emptype==1 | emptype==4  // private employee including unpaid family workers
			...
			egen no_no_school	   =total(edu_group_1==1), by(hhid survey)
			egen no_primary 	   =total(edu_group_2==1), by(hhid survey)
			egen no_secondary      =total(edu_group_3==1), by(hhid survey)
			egen no_tertiary       =total(edu_group_4==1), by(hhid survey)

			tab occupation, gen(occupation_)
			egen no_unemployed 	 =total(occupation_1==1), by(hhid survey)
			egen no_self_employed=total(occupation_2==1), by(hhid survey)
			egen no_employer     =total(occupation_3==1), by(hhid survey)
			egen no_employee     =total(occupation_4==1 | occupation_5==1), by(hhid survey)
...
global head_char "married age_group_2-age_group_5 edu_group_2-edu_group_4 sector_2-sector_9 occupation_3-occupation_6 emp_status_2 emp_status_3"
global hh_char "parish_2-parish_10 no_dependent no_unemployed no_employer no_no_school no_primary no_secondary no_tertiary members member_sq own_home wash_machine fridge telef n_bedroom car computer"
...
gen res_sim=.
	...
	preserve 
		...
		svyset ed [pw=nweight], strata(parish)
		svy: mean poverty_hat if survey=="LFS" & ln_pcexpae_def11_hat!=.
		mat B=e(b)
