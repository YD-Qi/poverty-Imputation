...
gen rent=0
replace rent=log(renta_imp/cpi11) if own_home==1 & renta_imp>0
replace rent=log(c0411100/cpi11) if own_home==0 & renta_imp>0
...
gen hr_work_a=0
replace hr_work_a=log(hstrp) if hstrp!=. & employed==1

gen hr_work=0
replace hr_work=log(hstrp) if hstrp!=. & employed==1
...
global head_char "sex relation hhid married age_group edu_group emp_status sector occupation unemployed emp_status hr_work hr_work_a want_work rent n_room poor_mat"
...
loc c=20
	...
	global head_char "sex age_group_2-age_group_5 edu_group_2-edu_group_4 sector_7-sector_9 occupation_2-occupation_4 emp_status_2 emp_status_3 want_work int_rent int_rent2 poor_mat n_room hr_work" 
...
local years "2016 2015 2014 2012 2011 2009 2008"
		...
		gen hr_work=0
		replace hr_work=log(hrs) if hrs!=. & employed==1

		gen hr_work_a=0
		replace hr_work_a=log(hrs_a) if hrs_a!=. & employed==1
		...
		gen rent=0
		replace  rent=log(c0421101/cpi11) if own_home==1 & (year==2008|year==2009)
		replace  rent=log(c0411100/cpi11) if own_home==0 & (year==2008|year==2009)
		
		replace  rent=log(h4/cpi11) if own_home==1 & (year>2009)
		replace  rent=log(h5/cpi11) if own_home==0 & (year>2009)		
		...
		global hh_char "no_dependent urban parish members member_sq own_home wash_machine fridge telef car computer tv n_bedroom int_rent int_rent2 poor_mat n_room"
