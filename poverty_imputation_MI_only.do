...
egen no_no_school	   =total(edu_group_1==1), by(hhid year)
egen no_primary 	   =total(edu_group_2==1), by(hhid year)
egen no_secondary      =total(edu_group_3==1), by(hhid year)
egen no_tertiary       =total(edu_group_4==1), by(hhid year)
...
global char_cont "ln_pcexpae_def11 n_bedroom no_dependent no_unemployed no_employer no_no_school no_primary no_secondary no_tertiary members member_sq"
global char_bi "married own_home wash_machine fridge telef car computer"
global char_cat_uno "sector emp_status occupation"
global char_cat_o "edu_group age_group"

// impute missing values for all necessary variables
mi set mlong
mi register imputed  $char_cont $char_bi $char_cat_uno $char_cat_o
qui mi impute chained (regress) $char_cont (logit) $char_bi (mlogit) $char_cat_uno (ologit) $char_cat_o = sex [pw=weight], augment add(5) rseed(1234) force noisily 
...
bys year hhid: replace ln_pcexpae_def11=mean(ln_pcexpae_def11)
mi xeq: gen pcexpae_def11_hat=exp(ln_pcexpae_def11)
mi xeq: gen poverty_hat=(pcexpae_def11_hat <= povline_def11)
}
mi estimate: svy: mean poverty_hat if year==2016 & survey=="HBS"
dis `i'
mi estimate: svy: mean poverty_hat if year==`i' & survey=="LFS"
}																																																															
